...
import { Mail, MapPin, Shield, Clock, PlusCircle } from "lucide-react";

interface Props {
  employee: AllEmployeeType;
  user: UserType | null;
  updateEmployeeFreeDays: (
    user_id: string,
    free_days: number,
  ) => Promise<void> | void;
}
...
const EmployeeCard = ({ employee, user, updateEmployeeFreeDays }: Props) => {
  const [freeDaysInput, setFreeDaysInput] = useState<number>(
    employee.free_days,
  );
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <Shield
              size={12}
              className={
                employee.role === "admin" ? "text-rose-500" : "text-zinc-500"
              }
            />
            <span
        ...
        <div className="pt-4 border-t border-zinc-800 relative z-10">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 text-xs font-bold py-2 px-4 rounded-xl transition-all active:scale-95 bg-orange-500 hover:bg-orange-600 text-white disabled:bg-zinc-800 disabled:text-zinc-600 disabled:cursor-not-allowed ${
